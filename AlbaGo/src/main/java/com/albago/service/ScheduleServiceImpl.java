...
//	}
	...
	public int whenInsertDummy(ScheduleRepeatVO scheduleRepeat) {
		...
		repeatEnd.set(now.get(Calendar.YEAR), now.get(Calendar.MONTH), 1, 0, 0, 0);
		repeatEnd.add(Calendar.MONTH, 3);
		...
		for (int i = 0; i < dayArr.length; i++) { // 요일 별 반복문
			...
			Calendar calStart = (Calendar) now.clone(); // 근무 시작 날짜와 시간
			calStart.set(2020, 0, 1);

			Calendar criteria = (Calendar) calStart.clone();
			criteria.set(Calendar.HOUR_OF_DAY, 0);
			criteria.set(Calendar.MINUTE, 0);
			...
			log.info("dayArrLoopStart" + ForLog.dot);
			log.info("dayArr_int: " + dayArr_int);
			log.info("calStartDefault: " + calStart.getTime());
			...
			if (dayArr_int >= criteria.get(Calendar.DAY_OF_WEEK)) { // 주어진 요일이 오늘의 요일보다 늦다면 그 날엔 일정 추가
	...
	public int insertDummy(ScheduleRepeatVO scheduleRepeat) {
		...
		if (result == 1) {
			whenInsertDummy(scheduleRepeat);
		}
		return result;
	}
